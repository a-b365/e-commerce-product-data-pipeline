...
def standardize(df):
   ...
   group_a = df.index[(df['category'] == "Electronics|Computers & Accessories|Computers & Tablets|Laptops|Traditional Laptops") | 
   ...
   group_b = df.index[(df['category'] == "CDs & Vinyl|Classic Rock|Album-Oriented Rock (AOR)") | 
   ...
   group_c = df.index[(df['category'] == "Clothing, Shoes & Jewelry|Men|Clothing|Shirts|T-Shirts") |
...
if __name__  == "__main__":
   ...
   #If rating is NaN full with mode of the column
   ...
   #If review_count is NaN fill with min of the column
   df["review_count"].fillna(value=df["review_count"].min(), inplace=True)
